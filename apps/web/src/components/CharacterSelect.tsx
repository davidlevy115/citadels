  ...
  return (
    ...
    >
      <div className="bg-slate-800 rounded-xl p-6 max-w-4xl w-full mx-4 border border-slate-600 max-h-[90dvh] overflow-y-auto">
        <h2 className="text-xl font-bold text-center mb-2">{t('select.title')}</h2>
        <p className="text-slate-400 text-sm text-center mb-3">{t('select.subtitle')}</p>
                ...
                {p.city.length > 0 && (
                  <div className="flex flex-wrap gap-0.5 max-w-[60px]">
                    {p.city.map((d, idx) => (
                      <div
                        key={idx}
                        title={`${t.district(d.name)} (${t.districtType(d.type)})`}
        ...
        {removedFaceUp.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-slate-500 mb-2 text-center">{t('select.removedFaceUp')}</p>
        ...
        {(warrantedRanks.length > 0 || threatenedRanks.length > 0) && (
          <p className="text-[11px] text-slate-400 text-center mb-3">
            ...
            {threatenedRanks.length > 0 && (
              <span>
                <span className="text-rose-400">✉</span> {t('select.threatOn', { ranks: [...threatenedRanks].sort((a, b) => a - b).join(', ') })}
              </span>
            )}
          </p>
        )}
                ...
                {(warrantedRanks.includes(char.rank) || threatenedRanks.includes(char.rank)) && (
                  <div className="absolute -top-1.5 -right-1.5 flex gap-0.5 z-10">
                    {warrantedRanks.includes(char.rank) && (
                      <span title={t('select.warrantMarker')}
                    ...
                    {threatenedRanks.includes(char.rank) && (
                      <span title={t('select.threatMarker')}
